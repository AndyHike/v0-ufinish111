...
function normalizePhoneInput(phone) {
  ...
  if (typeof phone === "string") {
    ...
    // Profiles sometimes store the phone as a JSON-encoded array string, e.g.
    // '["420775848259"]'. Sending that literally makes RO App reject the
    // contact with HTTP 400, so parse it back into a real array first.
    if (trimmed.startsWith("[")) {
      try {
        ...
        // Not valid JSON — fall through and treat it as a plain phone string.
      }
    }
...
function buildPhones(phone) {
  const phones = normalizePhoneInput(phone)
