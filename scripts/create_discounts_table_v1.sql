-- Create discounts table with flexible scope support
CREATE TABLE IF NOT EXISTS discounts (
  ...
  -- Scope of discount (what it applies to)
  scope_type VARCHAR(50) NOT NULL CHECK (scope_type IN ('service', 'brand', 'series', 'model', 'all_services', 'all_models')),
  ...
  CONSTRAINT valid_scope CHECK (
    (scope_type = 'service' AND service_id IS NOT NULL) OR
    ...
    (scope_type IN ('all_services', 'all_models'))
  ),
...
COMMENT ON TABLE discounts IS 'Flexible discount system supporting different scopes (service, brand, series, model)';
COMMENT ON COLUMN discounts.scope_type IS 'Type of scope: service (specific service), brand (all services for brand), series (all models in series), model (specific model), all_services, all_models';
COMMENT ON COLUMN discounts.discount_type IS 'Type of discount: percentage (e.g., 15%) or fixed (e.g., 500 CZK)';
COMMENT ON COLUMN discounts.discount_value IS 'Discount amount - either percentage (1-100) or fixed amount in currency';
