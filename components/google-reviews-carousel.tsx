...
export function GoogleReviewsCarousel({ data }: GoogleReviewsCarouselProps) {
  ...
  const formatDate = (timestamp: number, locale: string) => {
    try {
      // Map locales to appropriate format
        ...
        <div className="text-center mb-12">
          ...
          {hasReviews ? (
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-gray-600">({data!.totalReviews} {t("reviews")})</span>
            </div>
          ) : (
              ...
              <div className="flex gap-4 px-4 min-w-min">
                {data!.reviews.map((review, index) => (
                  <ReviewCard
                    ...
                    formatDate={(ts) => formatDate(ts, (typeof window !== 'undefined' && localStorage.getItem('NEXT_LOCALE')) || 'en')}
                  />
                ))}
              </div>
            </div>
  ...
  return (
    <Card className="flex-none w-full md:flex-1 h-full hover:shadow-lg transition-shadow">
      <CardContent className="p-6 flex flex-col h-full">
        <div className="mb-4">
          <h3 className="font-semibold truncate text-sm">{review.author_name || t("unknownAuthor")}</h3>
