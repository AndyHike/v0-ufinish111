  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (!brandId && createMissing && row.brandName) {
          brandId = await findOrCreateBrand(supabase, row.brandName)
        }

        if (!brandId) {
          errors++
          errorMessages.push(`Рядок ${i + 1}: Відсутній бренд`)
          continue
        }
        ...
        if (row.existingId) {
          const { error } = await supabase.from("series").update(seriesData).eq("id", row.existingId)
