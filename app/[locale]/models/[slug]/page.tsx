        ...
        {transformedModelServices && transformedModelServices.length > 0 ? (
          <div className="grid gap-4">
            {transformedModelServices.map((modelService) => (
              <div
                key={modelService.id}
                className="flex items-center justify-between rounded-lg border p-6 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex-1">
                  <h3 className="text-xl font-medium mb-2">{modelService.service.name}</h3>
                  <p className="text-muted-foreground text-sm">{modelService.service.description}</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <Button asChild>
                    <Link href={`/${locale}/services/${modelService.service.id}?model=${model.slug}`}>
                      <ExternalLink className="h-4 w-4 mr-2" />
                      {commonT("viewService") || "Переглянути послугу"}
                    </Link>
                  </Button>
                </div>
              </div>
