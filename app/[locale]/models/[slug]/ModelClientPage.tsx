...
export default async function ModelClientPage({ params }: Props) {
  const { slug, locale } = params
  const t = await getTranslations({ locale, namespace: "Models" })
  const commonT = await getTranslations({ locale, namespace: "Common" })

  // Facebook Pixel tracking for service requests
  const trackServiceRequest = (serviceName: string, modelName: string, brandName: string) => {
                ...
                <div className="mt-4 flex justify-end">
                  <Button variant="outline" asChild>
                    <Link
                      ...
                      onClick={() =>
                        trackServiceRequest(modelService.service.name, model.name, model.brands?.name || "")
                      }
                    >
